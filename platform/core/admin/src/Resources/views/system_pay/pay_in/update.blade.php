							...
							<div class="form-group {{ $errors->first('pi_user_id') ? 'has-error' : '' }}">
								...
								<select name="pi_user_id" id="" class="form-control js-select2">
									@foreach($users as $user)
										<option value="{{ $user->id }}" {{ $payIn->pi_user_id == $user->id ? "selected='selected'" : "" }}>
											#{{ $user->id }} - {{ $user->email }} - {{ $user->phone }}</option>
									@endforeach
								</select>
						...
						<div class="col-sm-8">
							<div class="form-group {{ $errors->first('pi_provider') ? 'has-error' : '' }}">
								...
								<select name="pi_provider" id="" class="form-control">
									<option value="1" {{ $payIn->pi_provider == 1 ? "selected='selected'" : "" }}>Chuyển Khoản</option>
									<option value="2" {{ $payIn->pi_provider == 2 ? "selected='selected'" : "" }}>Momo</option>
								</select>
							</div>
						</div>
						<div class="col-sm-8">
							<div class="form-group {{ $errors->first('pi_money') ? 'has-error' : '' }}">
								...
								<input type="text" class="form-control" {{ $payIn->pi_status >= 2 ? "disabled" : "" }}  name="pi_money" value="{{ $payIn->pi_money }}"  placeholder="2,000,000">
